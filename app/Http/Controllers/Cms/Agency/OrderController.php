...
namespace App\Http\Controllers\Cms\Agency;
...
class OrderController extends Controller
{
    public function index()
    {
        ...
        // dd($service_times);
        return view('cms.agency.orders.list', compact(
            'orders',
            'customers',
            'products',
            'currency',
    ...
    public function list($id = null)
    {
        ...
        $ops = $ops->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            // $duplicate_project = route('projects.agency.project.duplicate', $op->id);
            $pdf_route = route("cms.agency.orders.po.pdf", $op->id);
            $pdf_download = route("cms.agency.orders.po.pdf.download", $op->id);

            $div_action = '<div class="font-sans-serif btn-reveal-trigger position-static">';
            $profile_action =
                '<a href="' . route("cms.agency.orders.order", $op->id) . '" class="btn-table btn-sm"  data-id="' .
                $op->id .
    ...
    public function getLines($id)
    {
        ...
        $view = view('/cms/agency/orders/_order_details_modal', [
            'order' => $order,
            'lines' => $lines,
    ...
    public function get($id)
    {
        ...
        $view = view('/procurement/agency/purchase/mv/edit', [
            'purchase' => $purchase,
            'customers' => $customers,
            'Products' => $Products,
            'currency' => $currency,
    ...
    public function viewPo($id)
    {
        ...
        return view('procurement.agency.purchase.purchase-order', [
            'header' => $header,
            'lines' => $lines,
            'company' => $company,
        ]);
    ...
    public function store(Request $request)
    {
        ...
        // return redirect()->route('projects.agency.project')->with($notification);
    } // store
...
')
    ...
    public function switch($id)
    {
        if ($id) {
            if (Event::findOrFail($id)) {
                ...
                return redirect()->route('cms.agency.orders')->with('message', 'Event Switched.');
            } else {
    ...
    public function pickEvent(Request $request)
    {
        ...
        // return view('mds.agency.booking.pick', compact('events'));
        if ($request->event_id) {
            ...
            if (Event::findOrFail($request->event_id) && !session()->has('EVENT_ID')) {
                ...
                return redirect()->route('cms.agency')->with('message', 'Event Switched.');
        ...
        return redirect()->route('cms.agency')->with('error', 'Event not found.');
        // }
    }
