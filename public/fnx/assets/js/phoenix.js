(function (global, factory) {
  ...
  const choicesInit = () => {
    ...
    if (window.Choices) {
      ...
      elements.forEach(item => {
        ...
        // console.log('returning choices instance')
  ...
  const flatpickrInit = () => {
    console.log('inside flatpickrInit')
    const { getData } = window.phoenix.utils;
  ...
  const sortableInit = () => {
    ...
    const defaultOptions = {
      ...
      onEnd: function(evt) {
        ...
        let task_id = itemEl.dataset.id;
        let new_status_id = to.dataset.id;

        console.log('to', to.dataset.id);
        console.log('task_id',task_id );
        console.log('new_status_id',new_status_id );
        $.ajax({
          url: "/projects/admin/board/update" ,
          type: "POST",
          headers: {
              "X-CSRF-TOKEN": $('input[name="_token"]').attr("value"), // Replace with your method of getting the CSRF token
          },
          data: {task_id:task_id, new_status_id:new_status_id},
          // dataType: "json",
