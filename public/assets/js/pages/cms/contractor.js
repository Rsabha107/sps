$(document).ready(function () {
    ...
    // ************************************************** task venues
    $(".js-select-event-assign-multiple-add_venue_id, .js-select-event-assign-multiple-edit_venue_id").select2({
        closeOnSelect: false,
        placeholder: "Select ...",
    });

    $(".js-select-event-assign-multiple-add_event_id, .js-select-event-assign-multiple-edit_event_id").select2({
        closeOnSelect: false,
        placeholder: "Select ...",
    });
    ...
    $("#add_event_idx, #edit_event_idx").on("change", function () {
        ...
        if (eventId) {
            ...
            $.ajax({
                url: "/cms/setting/contractor/get/venues/" + eventId,
                method: "GET",
                async: true,
    ...
    $("body").on("click", "#editContractors", function () {
        ...
        $.ajax({
            url: "/cms/setting/contractor/get/" + id,
            type: "get",
            headers: {
            ...
            success: function (response) {
                ...
                drEvent.settings.defaultFile = `/storage/contractor/logo/${imageUrl}`;
                drEvent.destroy();
                drEvent.init();
...
$("body").on("click", "#deleteContractor", function (e) {
    var id = $(this).data("id");
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/cms/setting/contractor/delete/" + id,
                type: "DELETE",
                headers: {
                    "X-CSRF-TOKEN": $('input[name="_token"]').attr("value"), // Replace with your method of getting the CSRF token
                },
...
function imageFormatter(value, row, index) {
    if (!value)
        return `<img src="/storage/contractor/logo/noimage.png" alt="product image" width="60" class="rounded-circle pull-up">`;
    return `<img src="/storage/contractor/logo/${value}" alt="product image" width="60" class="rounded-circle pull-up">`;
}
